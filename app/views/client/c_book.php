...
// (Optional) if you pass these from controller, otherwise JS uses its hardcoded rates
$servicePriceRates = $data['servicePriceRates'] ?? [];
$timePriceModifier = $data['timePriceModifier'] ?? [];

// 🚨 HARD STOP if caretaker data missing
...
// min booking date (keep consistent with JS + controller)
    ...
    <section class="booking-form">
        <form id="bookingForm" method="POST" action="<?= URLROOT ?>/public/?url=client/bookCaretaker">
            ...
            <div class="form-group">
                ...
                <input type="text" name="district" id="district" value="<?= htmlspecialchars($ct['location'] ?? '') ?>" readonly>
            ...
            <div class="form-group">
                ...
                <small class="hint">
                    Note: Additional customization requests may result in a change to the final price after review.
                </small>
            </div>
...
<script>
    ...
    // JS will read service type from #service_type too
    window.SERVICE_TYPE = "<?= htmlspecialchars($serviceType, ENT_QUOTES) ?>";

    // Optional if you want JS to use server-provided rates.
    // If empty, JS hardcoded defaults will still work.
    window.SERVICE_PRICE_RATES = <?= json_encode($servicePriceRates) ?>;
    window.TIME_MODIFIERS = <?= json_encode($timePriceModifier) ?>;
</script>
