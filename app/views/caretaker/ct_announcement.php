...
<head>
    ...
    <link rel="stylesheet" href="<?= URLROOT ?>/public/css/caretaker/ct_announcement.css">
...
<body>
    ...
    $caretaker_audience_label = static function (string $role): string {
        $r = strtolower($role);
        if ($r === 'caretaker') {
            return 'Caregivers';
        }
    ...
    <main class="main-content admin-dashboard-page caretaker-announcements-page">
        ...
        <?php else: ?>
            <div class="table-container caretaker-announcements-table-wrap">
                <table class="table booking-table caretaker-announcements-table client-table" data-table-collapse="off">
                    <thead>
                        <tr>
                            <th>Date</th>
                            <th>Title</th>
                    ...
                    <tbody>
                        <?php foreach ($announcements as $ann): ?>
                            ...
                            ?>
                            <tr>
                                <td class="caretaker-announcements-date"><?= ct_ann_esc($dateOut) ?></td>
                                <td class="caretaker-announcements-title">
                                    <span class="caretaker-announcements-title-text"><?= ct_ann_esc($title) ?></span>
                                    <?php if ($by !== ''): ?>
                                        <span class="caretaker-announcements-by">By <?= ct_ann_esc($by) ?></span>
                                    <?php endif; ?>
                                </td>
                                <td class="caretaker-announcements-msg" title="<?= ct_ann_esc($msg) ?>"><?= ct_ann_esc($ann_preview($msg, 200)) ?></td>
                                <td><span class="status-pill caretaker-announcements-audience"><?= ct_ann_esc($caretaker_audience_label($role)) ?></span></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
