/**
 * HR booking reschedule requests — detail modal; approve / reject POST to existing routes.
 */
(function () {
    'use strict';
    ...
    var DETAIL_FIELDS = [
        ...
        ['old_date', 'Requested from date'],
        ['new_date', 'Requested new date'],
        ['reason', 'Client reason'],
        ['booking_caretaker_id', 'Assigned caregiver ID'],
        ['caretaker_name', 'Assigned caregiver name'],
        ['service_type', 'Service type'],
        ['basis', 'Basis'],
        ['duration', 'Duration (units)'],
        ['booking_date', 'Current booking date'],
        ...
        ['hr_note', 'HR note'],
        ['created_at', 'Request submitted at'],
        ['reviewed_at', 'Request reviewed at']
    ];

    function getEndpoints() {
        var el = document.getElementById('hr-reschedule-requests-endpoints');
        if (!el) {
    ...
    function fillDetailModal(d) {
        var dl = document.getElementById('rescheduleDetailDl');
        var titleEl = document.getElementById('rescheduleDetailTitle');
        if (!dl) {
            return;
        }
        dl.innerHTML = '';
        ...
        if (titleEl) {
            ...
            titleEl.textContent = rid ? 'Reschedule request #' + rid : 'Reschedule request';
        }
    }
    ...
    function parseRow(el) {
        var raw = el.getAttribute('data-reschedule-row') || '{}';
        try {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        var detailModal = document.getElementById('rescheduleDetailModal');
        var approveModal = document.getElementById('rescheduleApproveModal');
        var rejectModal = document.getElementById('rescheduleRejectModal');
        var approveNote = document.getElementById('rescheduleApproveNote');
        var rejectNote = document.getElementById('rescheduleRejectNote');
        ...
        document.querySelectorAll('[data-close-reschedule-detail]').forEach(function (btn) {
        ...
        var approveCancel = document.getElementById('rescheduleApproveCancel');
        var approveSubmit = document.getElementById('rescheduleApproveSubmit');
        var rejectCancel = document.getElementById('rescheduleRejectCancel');
        var rejectSubmit = document.getElementById('rescheduleRejectSubmit');
