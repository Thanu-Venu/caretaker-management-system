document.addEventListener("DOMContentLoaded", () => {
  ...
  // time element may be select or input; keep reference later
  let preferredTimeSelect = document.getElementById("preferredTime");
  ...
  const customizationHoursInput = document.getElementById(
    "customization_hours",
  );
  ...
  function calculatePrice() {
    ...
    const customizationHours =
      parseInt(customizationHoursInput?.value || "0", 10) || 0;
    ...
    if (basis && duration > 0 && preferredTime) {
      ...
      if (basePriceLabel)
        basePriceLabel.textContent = baseCost.toLocaleString();
      if (basePriceAmount)
        basePriceAmount.textContent = baseCost.toLocaleString();
      if (customizationPriceSpan)
        customizationPriceSpan.textContent = customizationFee.toLocaleString();
  ...
  // helper to ensure preferredTimeSelect is a select element
  function ensureTimeSelect() {
    let elem = document.getElementById("preferredTime");
    if (elem && elem.tagName.toLowerCase() === "select") {
      preferredTimeSelect = elem;
      return elem;
    }
    ...
    preferredTimeSelect = select;
    // if a prefill value exists, add and select it
    if (preferredTimeValue) {
      const opt = document.createElement("option");
      opt.value = preferredTimeValue;
      opt.textContent = preferredTimeValue;
      select.appendChild(opt);
      select.value = preferredTimeValue;
    }
    return select;
  }

  // on load, if serviceType is Maid and basisValue hourly, switch
  if (serviceType === "Maid" && basisValue === "Hourly") {
    if (preferredTimeLabel) preferredTimeLabel.textContent = "Start Time";
    if (timeContainer) {
      const input = document.createElement("input");
      input.type = "time";
      input.id = "preferredTime";
      input.required = true;
      if (preferredTimeValue) input.value = preferredTimeValue;
      timeContainer.innerHTML = "";
      timeContainer.appendChild(input);
      preferredTimeSelect = input;
    }
  }

  basisSelect.addEventListener("change", calculatePrice);
  durationInput.addEventListener("input", calculatePrice);
  // attach generic listener to container (handles both select and input)
